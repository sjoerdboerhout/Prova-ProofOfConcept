 ...
 * 
 * Run Prova from the command line. - Parse command line arguments - Set log options - Create a Prova instance -
 * Configure the Prova instance - Via properties file(s) - Via command line arguments - Execute Prova - Wait until Prova
 * finished
 * 
...
public class Cli extends ProvaRunner {
    ...
    /**
     * Create, initialize and start a Cli instance. Command line arguments overrule all other properties
     * 
     * @param args
     */
    ...
    /**
     * Create and configure an instance of Prova - Parse options and arguments - Detect Prova root path - Handle command
     * line arguments - Configure Prova - Hardcoded defaults from resource file - User defaults file (optional) - User
     * defaults test file (optional) - Project file (optional) - Project test file (optional) - User provided file
     * (optional) - Provided CLI arguments (optional)
     * 
     * @param args
     * @throws Exception
     */
        ...
        try {
            ...
            if (cliProperties.containsKey(Config.PROVA_CONF_FILE_USER)) {
                LOGGER.debug("Load user defined property file '{}'",
                        () -> cliProperties.getProperty(Config.PROVA_CONF_FILE_USER));
    ...
    private Properties parseCliOptions(String[] args, Options options) throws Exception {
        ...
        try {
            ...
            if (cmdLine.hasOption("config"))
                properties.setProperty(Config.PROVA_CONF_FILE_USER, cmdLine.getOptionValue("config"));
            if (cmdLine.hasOption("env"))
                properties.setProperty(Config.PROVA_ENV, cmdLine.getOptionValue("env"));
            if (cmdLine.hasOption("filters"))
                properties.setProperty(Config.PROVA_TESTS_FILTERS, cmdLine.getOptionValue("filters"));
            if (cmdLine.hasOption("loglevel"))
                properties.setProperty(Config.PROVA_LOG_LEVEL, cmdLine.getOptionValue("loglevel"));
            if (cmdLine.hasOption("out"))
                properties.setProperty(Config.PROVA_PLUGINS_REPORTING_FILE, cmdLine.getOptionValue("out"));
            if (cmdLine.hasOption("plugins"))
                properties.setProperty(Config.PROVA_CLI_PLUGINS, cmdLine.getOptionValue("plugins"));
            if (cmdLine.hasOption("root"))
                properties.setProperty(Config.PROVA_TESTS_ROOT, cmdLine.getOptionValue("root"));
            if (cmdLine.hasOption("start"))
                properties.setProperty(Config.PROVA_TESTS_START, cmdLine.getOptionValue("start"));
            if (cmdLine.hasOption("timeout"))
                properties.setProperty(Config.PROVA_TIMEOUT, cmdLine.getOptionValue("timeout"));
            if (cmdLine.hasOption("uitvoeren"))
                properties.setProperty(Config.PROVA_TESTS_EXECUTE, cmdLine.getOptionValue("uitvoeren"));
     ...
     * @throws Exception
     */
    private void configureCliOption(String shortOpt, String longOpt, Boolean hasArgs, String argName,
            char valueSeparator, String description) throws Exception {
        try {

            LOGGER.trace("Add new cli option '{}'::'{}'", shortOpt, longOpt);

            Option option = Option.builder(shortOpt).longOpt(longOpt).required(false).hasArg(hasArgs).argName(argName)
