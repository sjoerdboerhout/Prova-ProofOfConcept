...
public class Prova implements Runnable, TestRunner {
    ...
    /**
     * Set up the Prova runner. The argument 'project' indicates which project Prova starts.
     * 
     * @param project
     */
    ...
    /**
     * Called when the Prova thread starts. Run all steps of the test process in correct order
     */
    @Override
    public void run() {
        try {
    ...
    /**
     * Initialize Prova and set everything ready for test execution. - Read projects properties file(s) - Check for a
     * valid input plug-in - Check for a valid output plug-in - Check for (a) valid report plug-in(s)
     */
    private void init() throws Exception {
        ...
        try {
            ...
            pluginLoader.addFiles(
                    properties.getProperty(Config.PROVA_DIR) + properties.getProperty(Config.PROVA_OS_FILE_SEPARATOR)
            ...
            LOGGER.debug("Load and initialize web output plug-in '{}'",
            ...
            LOGGER.debug("Load and initialize SOAP webservice output plug-in '{}'",
            ...
            LOGGER.debug("Load and initialize DB output plug-in '{}'",
            ...
            LOGGER.debug("Load and initialize SHELL output plug-in '{}'",
    ...
    /**
     * Index and prepare test scripts for test execution. - Search for test scripts - Create structure of test suites
     * and test cases - Only read the 'headers' and not the full test cases - Execute the (optional) setup commands to
    ...
    private boolean checkTestCaseFilter(TestCase testCase) {
        ...
        if (labelFilterString != null && !"".equals(labelFilterString.trim())) {
            ...
            if (labelFilters.size() > 0) {
                LOGGER.debug("Checking label filters '{}' for testcase with labels {}.", labelFilterString,
                        testCase.getLabels());
    ...
    /**
     * TearDown after test execution - Execute the (optional) tear down commands to reset the test environment
     */
    ...
    /**
     * Prepare Prova for exit - Provide a test summary - Shutdown input plug-in - Shutdown output plug-in - Shutdown
     * report plug-in(s)
     */
    ...
    /**
     * Set the root test suite. This test suite should contain the whole structure of test suites and test cases
     * 
     * @param rootTestSuite
     */
    @Override
    ...
    public String getPropertyValue(String key) throws Exception {
        LOGGER.trace("Get value of property: '{}' ({})", () -> key,
    ...
    public String replaceKeywords(String entry) throws Exception {
        ...
        while (matcher.find()) {
            ...
            if (!this.hasPropertyValue(keyword)) {
                LOGGER.trace(
                        "No value found for property " + keyword + ", assuming it will be available at execute time.");
                continue;
            }
