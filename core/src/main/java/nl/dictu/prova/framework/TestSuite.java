...
/**
 * Contains a tree of (sub) test suites and test cases. This tree holds all information about test suites and test cases
 * and offers an interface to execute these tests via the execute-function.
 * 
...
public class TestSuite {
    ...
    /**
     * Constructor for a test suite without a parent. The id is a unique identifier for the test suite.
     * 
     * @param id
     * @throws Exception
     */
    ...
    /**
     * Constructor for a test suite with a parent. The id is a unique identifier for the test suite. The parent is
     * another test suite.
     * 
     * @param id
     * @param parent
    ...
    /**
     * Set the unique identifier of this test suite. This identifier must be at least 1 character long and is required
     * to be unique in the whole set.
     * 
     * @param testSuiteId
     * @throws Exception
     */
    ...
    /**
     * Get the number of child test suites of this test suite. When countSubTestSuites is set to true all sub-suites are
     * counted too.
     * 
    ...
    /**
     * Check if a test case is part of this test suite. When searchSubTestSuites is set to true all sub-suites are
     * searched too.
     * 
     * @param testSuite
    ...
    public boolean hasTestSuite(TestSuite testSuite, boolean searchSubTestSuites) {
        LOGGER.trace("Test if this test suite has a child {} ({})", () -> testSuite.getId(),
                () -> testSuite.toString());
        ...
        for (Map.Entry<String, TestSuite> entry : testSuites.entrySet()) {
            ...
            if (searchSubTestSuites && entry.getValue().hasTestSuite(testSuite, searchSubTestSuites)) {
                return true;
            }
        }
    ...
    /**
     * Get the number of test cases of this test suite. When countSubTestCases is set to true all sub-cases are counted
     * too.
     * 
    ...
    public String toString() {
        return String.format("ID: %s%s (TC: %d, TS: %d)", id, parent != null ? ", parent: " + parent.getId() : "",
                numberOfTestCases(), numberOfTestSuites());
    }
}
