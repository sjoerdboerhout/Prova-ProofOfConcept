...
public class Text extends Parameter {
    ...
    public boolean isValid(String text) {
        LOGGER.trace("Validate '{}' as a String. Min: {}, Max: {}", () -> text, () -> minLength, () -> maxLength);
    ...
    public void setMinLength(Integer minLength) throws Exception {
        if (minLength < MIN_LENGTH) {
            throw new Exception("Minimum value must be >= " + MIN_LENGTH + " (" + minLength + ")");
        }

        if (minLength > maxLength) {
            throw new Exception("New maximum length must be <= " + maxLength + " (" + minLength + ")");
        }
    ...
    public void setMaxLength(Integer maxLength) throws Exception {
        if (maxLength > MAX_LENGTH) {
            throw new Exception("Maximum value must be <= " + MAX_LENGTH + " (" + maxLength + ")");
        }

        if (maxLength < minLength) {
            throw new Exception("New maximum timeout must be >= " + minLength + " (" + maxLength + ")");
        }
