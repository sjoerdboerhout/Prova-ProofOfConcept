 ...
 * <p>
 */
package nl.dictu.prova.plugins.output.web.selenide;
...
/**
 * Driver for controlling Selenide
 * 
 * @author Robert Bralts
 * @since  2019-01-11
 *
 */
public class Selenide implements WebOutputPlugin
{
  ...
  public void prepareWebdriver() throws Exception
  {
    try
    {
      ...
      Configuration.browser = browserType.toLowerCase();
      System.setProperty("wdm.targetPath", testRunner.getPropertyValue(Config.PROVA_PLUGINS_DIR)+"/output");
      ...
      if(testRunner.hasPropertyValue(Config.PROVA_PLUGINS_OUT_WEB_BROWSER_PROXY))
      {
            ...
            //String[] proxyPart = testRunner.getPropertyValue(Config.PROVA_PLUGINS_OUT_WEB_BROWSER_PROXY).split(":");
            //Configuration.proxyHost = proxyPart[0];
            //Configuration.proxyPort = Integer.getInteger(proxyPart[1]);
            System.setProperty("wdm.proxy", testRunner.getPropertyValue(Config.PROVA_PLUGINS_OUT_WEB_BROWSER_PROXY));
      ...
      if(browserType.equalsIgnoreCase("FireFox"))
      {
        ...
        System.setProperty("webdriver.gecko.driver", fireFoxPath);
          Configuration.browserBinary = fireFoxPath;
        ...
        if(testRunner.hasPropertyValue(Config.PROVA_PLUGINS_OUT_WEB_BROWSER_BIN_GECKO))
        {
            ...
            Configuration.browserBinary =  testRunner.getPropertyValue(Config.PROVA_PLUGINS_OUT_WEB_BROWSER_BIN_GECKO);
      ...
      else if(browserType.equalsIgnoreCase("Chrome"))
      {
        ...
        //  Configuration.browserBinary = chromePath;
                  Configuration.browserCapabilities = cap;
      ...
      if(testRunner.hasPropertyValue(url))
      {
        ...
        Configuration.timeout = maxTimeOut;
	 ...
	 *
	 */
	protected void setBrowserResolution() {
  ...
  public void doSelect(String xPath, Boolean select) throws Exception
  {
    ...
    while(true)
    {
      try
      {
        SelenideElement element = findElement(xPath);
    ...
    while(true)
    {
      try
      {
        ...
        try
        {
            ...
            element.selectOptionContainingText(select);
        }
        catch(Exception ex)
        {
	        ...
	        try
	        {
                ...
                element.selectOptionByValue(select);
	        }
	        catch(Exception nseex)
	        {
  ...
  public void doSendKeys(String xPath, String keys) throws Exception
  {
    ...
    try
    {
      ...
      if (xPath.equalsIgnoreCase("/html/body"))
      {
	      ...
	      LOGGER.trace("Send keys to active element not supported for now");
	      //webdriver.switchTo().activeElement().sendKeys(keys);
	      //SelenideElement element = new SelenideElement;
	      //element.
  ...
  public void doSwitchScreen(String name) throws Exception 
  {
    ...
    if (currentWindow.equals(""))
    	{
    		...
    		currentWindow = WebDriverRunner.getWebDriver().getWindowHandle();
    	}
    ...
    if(name != null)
    {
      ...
      if(useName)
      {
          ...
          Set<String> availableWindows = WebDriverRunner.getWebDriver().getWindowHandles();
          if (!availableWindows.isEmpty()) { 
            for (String windowId : availableWindows) 
            { 
              if (switchTo().window(windowId).getTitle().startsWith(name))
              { 
...
//	}
    ...
    //SelenideWait wait = new SelenideWait(webdriver, iTimeOut,1);
    int count = 0;
    String value = "";
    Boolean bCheck = true;
    for(String check: lChecks)
	        ...
	        else
	        {
	        	...
	        	try
	        	{
	        		...
	        		Assert.assertTrue("The value \"" + value + "\" is not found in the text: " + text,
	 			        //   wait.until(Condition.focused.textToBePresentInE0lement(element, value)));
    ...
    while(true)
    {
      try
      {
        if (alert)
        {
            ...
            LOGGER.error("Alerts not supported");
        ...
        else
        {
	        ...
	        else
	        {
		        ...
		        LOGGER.trace("Switching to frame '{}' (doSwitchFrame)", xPath);
                com.codeborne.selenide.Selenide.switchTo().frame(element);
	        } 
        }
        // Action succeeded. Return.
        return;
  ...
  public void doWaitForElement(String xPath, String type, Boolean value, double timeOut) throws Exception
  {
        ...
        LOGGER.debug("Waiting for element is not needed in Selenide, skipping this action");
	    /*int iTimeOut = 0;
	    try
	    {
	    	if (timeOut == 0)
  ...
  private void scroll_element_into_view(SelenideElement element) {
  ...
  private SelenideElement findElement(String xPath)
  {
    /*if(this.webdriver == null)
    {
      try
