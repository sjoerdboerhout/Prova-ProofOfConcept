...
public class Selenium implements WebOutputPlugin {
    ...
    public void prepareWebdriver() throws Exception {
        try {
            if (browserType.equalsIgnoreCase("FireFox")) {
                ...
                String iePath = testRunner.getPropertyValue(Config.PROVA_DIR) + File.separator
            ...
            String url = Config.PROVA_ENV_PFX + "." + testRunner.getPropertyValue(Config.PROVA_ENV).toLowerCase() + "."
    ...
    public void doCaptureScreen(String fileName) throws Exception {
        ...
        try {
            ...
            if (!new File(fileName).isFile() && !fileName.contains(":")) {
                fileName = testRunner.getPropertyValue(Config.PROVA_PLUGINS_REPORTING_DIR) + File.separator
                        + "Screenshots" + File.separator + fileName;
                LOGGER.debug("Pad2: " + fileName);
                if (!new File(fileName.substring(0, fileName.lastIndexOf(File.separator))).isFile()
                        && !fileName.contains(":")) {
             ...
             * fileName.lastIndexOf(File.separator))).isDirectory()||!fileName.contains(File.separator)) { fileName =
             * testRunner.getPropertyValue(Config.PROVA_PLUGINS_REPORTING_DIR)+ File.separator + "Screenshots" +
             * File.separator + fileName; LOGGER.debug("Pad2: " + fileName); } if(!new File(fileName.substring(0,
             * fileName.lastIndexOf(File.separator))).isDirectory()) { fileName =
             * testRunner.getPropertyValue(Config.PROVA_DIR) + File.separator + fileName; LOGGER.debug("Pad3: " +
             * fileName); } FileUtils.copyFile(scrFile, new File(fileName + ".png"));
    ...
    @Override
    public void doClick(String xPath, Boolean rightClick, Boolean waitUntilPageLoaded, Boolean continueOnNotFound)
            throws Exception {
        if (this.webdriver == null) {
            prepareWebdriver();
        }

        LOGGER.debug(">> Click with {} on '{}', Wait for page = {}", (rightClick ? "right" : "left"), xPath,
                waitUntilPageLoaded);
        ...
        while (true) {
            try {
                ...
                if (waitUntilPageLoaded) {
                    ...
                    // 30).until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(xPath)));
                    // new WebDriverWait(webdriver,
                    // 30).until(ExpectedConditions.not(ExpectedConditions.elementToBeClickable(element)));
                }
                ...
                LOGGER.debug("Exception while clicking on element '{}' retry count: '{}', Type: '{}' : '{}'", xPath,
                        count, eX.getClass().getSimpleName(), eX.getMessage());
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        throw new Exception("'" + select + "' can not be selected as 'text' or as 'value' in element: '"
                                + xPath + "'");
                    }
                }
    ...
    public void doSetText(String xPath, String text, Boolean replace) throws Exception {
        ...
        while (true) {
            try {
                ...
                if (++count > maxRetries) {
                    LOGGER.debug("Exception while setting text '{}' in '{}': {} (retry count: {})", xPath, text,
                            eX.getMessage(), count);
    ...
    public void doSleep(long waitTime) throws Exception {
        /*
         * if(this.webdriver == null) { prepareWebdriver(); }
         */
    ...
    public void doSwitchScreen(String name) throws Exception {
        ...
        boolean failOnError = Boolean
                .parseBoolean(testRunner.getPropertyValue(Config.PROVA_PLUGINS_OUT_WEB_BROWSER_FAILSWTCHSCR));
                ...
                try {
                    ...
                    if (!exists) {
                        ...
                        try {
                            ...
                            Assert.assertTrue("The value \"" + value + "\" is found in the text: " + text,
                                    wait.until(ExpectedConditions
                                            .not(ExpectedConditions.textToBePresentInElement(element, value))));
                        ...
                        // catch(TimeoutException eX)
                        {
                            ...
                            /*
                             * try {
                             * LOGGER.trace("Check if the attribute @value, in element {}, doesn't contain the text {}",
                             * xPath,value ); Assert.assertTrue("The value \"" + value + "\" is found in the text: " +
                             * text, wait.until(ExpectedConditions.not(ExpectedConditions.textToBePresentInElementValue(
                             * element, value))));
                             * //wait.until(ExpectedConditions.textToBePresentInElementValue(element, value))); }
                             * catch(AssertionError | TimeoutException e) //catch(TimeoutException e) {
                             * //this.doCaptureScreen("doValidateText"); throw new TimeoutException("The value \"" +
                             * value + "\" is found in the text: " + text); //throw e; }
                             */
                        }
                    }
                    // Check if element contains the given text
                        ...
                        // catch(TimeoutException eX)
                        {
                            ...
                            try {
                                LOGGER.trace("Check if the attribute @value, in element {}, contains the text {}",
                                        xPath, value);
                                Assert.assertTrue("The value \"" + value + "\" is not found in the text: " + text,
                                        wait.until(ExpectedConditions.textToBePresentInElementValue(element, value)));
                            ...
                            // catch(TimeoutException e)
                            {
                                ...
                                throw new TimeoutException(
                                        "The value \"" + value + "\" is not found in the text: " + text);
                            }
                        }
                    }
                    ...
                    if (++count > maxRetries) {
                        LOGGER.debug("Exception while validating text '{}' in '{}': {} (retry count: {})", value, xPath,
                                eX.getMessage(), count);
                        this.doCaptureScreen("doValidateText");
    ...
    @Override
    public void doStoreText(String xPath, String regex, String inputtext, String name, double timeOut)
            throws Exception {
        if (inputtext != null) {
    ...
    @Override
    public void doSwitchFrame(String xPath, Boolean alert, Boolean accept, String username, String password)
            throws Exception {
        if (this.webdriver == null) {
            prepareWebdriver();
        }
        ...
        while (true) {
            try {
                if (alert) {
                    ...
                    if (accept) {
                        ...
                        try {
                            ...
                            LOGGER.trace(
                                    "Accepting alert failed, maybe it is closed when passing user/password? (doSwitchFrame)");
                        }
                    } else if (!accept) {
                ...
                if (++count > maxRetries) {
                    LOGGER.debug("Exception while switching to frame '{}' : {} (retry count: {})", xPath,
                            eX.getMessage(), count);
        ...
        try {
            ...
            case ("exists"):
                ...
                if (value) {
                    ...
                    wait.until(
                            ExpectedConditions.not(ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath(xPath))));
                }
                break;
            case ("clickable"):
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    LOGGER.debug("Element is NOT clickable after waiting " + timeOut
                            + " seconds. Trying to loacate element" + "anyway, maybe it's hidden");
                }
                ...
                if (++count > maxRetries) {
                    ...
                    LOGGER.trace("Exception while searching element '{}' retry count: '{}', Type: '{}' : '{}'", xPath,
                            count, eX.getClass().getName(), eX.getMessage());
                }
            }
        }
    }
