...
public class SimpleReport implements ReportingPlugin {
	...
	public void setUp(String projectName) throws Exception {
		try {
			...
			// Save the test root to strip that part from the test suite/case
			// names.
			...
			/*
			 * - Check if property 'prova.plugins.reporting.dir' is an existing
			 * dir. - if not: Try to create it as a sub-dir in the Prova root
			 * dir.
			 */
			...
			if (checkValidReportDir(dir)) {
				...
				// Try if the configured path is a sub-directoy of the Prova
				// root path
				if (checkValidReportDir(dir)) {
					LOGGER.debug("Set up output directory for reporting to '" + dir.getAbsolutePath() + "'");
				} else {
			...
			pwSummary
					.println("<style> table, td { 	border: 1px solid black;	border-collapse: collapse;	font-family: Verdana, Helvetica, sans-serif;	font-size: 15px;}"
	...
	public void logStartTest(TestCase testCase) throws Exception {
		try {
			...
			currTestCaseFile = currTestSuiteDir + File.separator + testCaseFilename + File.separator
					+ testCaseSheetname + (testCaseDataSetFileName.length() > 0 ? File.separator : "")
					+ testCaseDataSetFileName + (testCaseDataSetSheetName.length() > 0 ? File.separator : "")
					+ testCaseDataSetSheetName + (testCaseDataSetColumnName.length() > 0 ? File.separator : "")
					+ testCaseDataSetColumnName + ".html";
	...
	public void shutDown() {
		...
		try {
			...
			pwSummary.println("<br><b>Testcases Processed: </b>" + (countPassedTestcases + countFailedTestcases)
					+ "</br>");
	...
	public void logStartTestSuite(TestSuite testSuite) throws Exception {
		...
		if (testSuiteSubDir.length() > 0) {
			...
			if (testSuite.numberOfTestCases(false) > 0) {
				...
				pwTestsuite.println("<h1>" + testSuite.getId().substring(testSuite.getId().lastIndexOf("\\") + 1)
						+ "</h1>");
