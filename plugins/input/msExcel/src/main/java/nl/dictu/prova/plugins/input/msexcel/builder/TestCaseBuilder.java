...
public class TestCaseBuilder {
    ...
    /**
     * Parse the given sheet. Find tags and execute related actions to retrieve/set the data for this test case.
     *
     * @param testCase
     * @param sheet
     * @throws Exception
    ...
    /**
     * Parse test actions and add the actions found to the correct action list. (Setup, action or teardown)
     *
     * @param testCase
     * @param sheet
     * @param rowNum
    ...
    private List<TestAction> readTestActionsFromReference(TestCase testCase, Map<String, String> rowMap, String type)
        ...
        // In case the sheet is of type 'SOAP' or 'DB', (optional) testdata is collected and used for iteration over the
        // sheet.
        if (new SheetPrefixValidator(nextSheet).validate("SOAP")
            ...
            if (rowMap.containsKey("testdatasheet")) {
                if (!(rowMap.get("testdatasheet").isEmpty())) {
                    ...
                    if (testData.size() > 0) {
                        // If multiple sets of testdata are defined, the 'template' of the soap message or db query is
                        // ran with each set of testdata.
    ...
    /**
     * Scan all rows on the given <sheet> and parse all actions and import referenced sheets.
     *
     * @param testActions
     * @param sheet
     * @param rowNum
            ...
            if (selectedProfileProperties == null) {
                ...
                if (!selectedProfileProperties.isEmpty()) {
                    for (Entry prop : selectedProfileProperties.entrySet()) {
                        ...
                        // Store all the profile properties in the central properties object after looking for possible
                        // property value
                        if (value == null)
                            continue;
