...
public class TestSuiteBuilder {
    ...
    private static final FileFilter excelFlowFileFilter = file -> !file.isDirectory() && !file.getName().startsWith("~")
            && Arrays.asList("xlsm").contains(FilenameUtils.getExtension(file.getName()));
    // private static final FileFilter excelDataFileFilter = file -> !file.isDirectory() &&
    // !file.getName().startsWith("~") && Arrays.asList("xlsx").contains(FilenameUtils.getExtension(file.getName()));
     ...
     * @param testCaseName
     */
    private LinkedList<String> collectDataSets(String flowFilePath, String flowName, String testCaseName,
            String testDataDir) {
        LinkedList<String> dataSetsList = new LinkedList<String>();

        FileFilter dataDirFilter = dir -> dir.isDirectory() && !dir.getName().startsWith("~")
                && dir.getName().endsWith(flowName);

        FileFilter dataFileFilter = file -> !file.isDirectory() && !file.getName().startsWith("~")
        ...
        try {
            ...
            LOGGER.error("Exception occured while retrieving the data sets for TC '{}'/'{}'", flowFilePath,
                    testCaseName, eX);
        } finally {
